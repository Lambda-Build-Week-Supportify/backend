...
router.get("/school/:id", (req, res) => {
  ...
  Schools.findSchoolById(school_id)
    .then(school => {
      if (school) {
        Schools.aSchoolUsers(school_id).then(school => {
          res.status(200).json(school);
        });
      } else {
        res.status(404).json({
          errorMessage:
            "Sorry, there's either no school by that id, or that school is not connected to any users"
        });
      }
    })
...
router.get("/user/:id", (req, res) => {
  ...
  Users.findById(user_id)
    .then(user => {
      if (user) {
        Schools.aUserSchools(user_id).then(user => {
...
router.post("/connect-user", restricted, (req, res) => {
  ...
  if (!board) {
        ...
        Schools.findSchoolById(school_id)
          .then(school => {
            if (!school) {
              res
                .status(404)
                .json({ errorMessage: "We don't have a school by that id" });
            } else {
              console.log(school);
              Schools.connectUser(user_id, school_id).then(info => {
    ...
    res
      .status(401)
      .json({
        message: "You do not have rights to add user-school relationship"
      });
  }
});
...
router.delete("/disconnect", restricted, (req, res) => {
  ...
  if (!board) {
        ...
        Schools.findSchoolById(school_id)
          .then(school => {
            if (!school) {
              ...
              console.log(school);
              Schools.removeUserFromSchool(user_id, school_id).then(info => {
                res.status(200).json({
                  info,
                  message:
                    "the user-school relationship was successfully deleted"
                });
              });
            }
          })
    ...
    res
      .status(401)
      .json({
        message: "You do not have rights to delete user-school relationship"
      });
  }
});
